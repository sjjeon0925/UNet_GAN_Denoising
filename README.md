...
```text
📦 Repository
├── 📂 models/                 # 모델 학습 및 테스트 노트북
│   ├── UNet_test.ipynb              # 1단계: U-Net 단독 모델
│   ├── UNet_GAN_test.ipynb          # 2단계: U-Net & GAN 결합 모델
│   └── UNet_GAN_Upgraded_test.ipynb # 3단계: U-Net & GAN 업그레이드 모델
├── 📂 data_processing/        # 데이터 전처리 스크립트
│   └── UNet_Noise_Data.ipynb        # 노이즈 파일 전처리
└── 📂 test_results/           # 검증 샘플(N=1~10) 테스트 결과물
    ├── 📂 audio/                    # 오디오 결과물 (.wav)
    └── 📂 figures/                  # 시각화 및 그래프 파일 (.png)
```

## 3. 단계별 모델 개발 및 테스트 결과

### 1단계: U-Net 단독 모델 (`models/UNet_test.ipynb`)
...
### 2단계: U-Net & GAN 결합 모델 (`models/UNet_GAN_test.ipynb`)
...
### 3단계: U-Net & GAN 업그레이드 모델 (`models/UNet_GAN_Upgraded_test.ipynb`)
